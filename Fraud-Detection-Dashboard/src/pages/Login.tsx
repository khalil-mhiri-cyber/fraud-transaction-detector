...
export default function Login() {
  ...
  const inputStyle = {
    width: '100%', padding: '11px 14px', background: 'rgba(8,13,26,0.85)',
    border: '1px solid rgba(148,163,184,0.12)', borderRadius: 4,
    fontFamily: 'Instrument Sans', fontSize: 14, color: '#e2e8f0',
      ...
      <div style={{ position: 'relative', width: 460, padding: '48px 40px', background: '#0d1528', border: '1px solid rgba(148,163,184,0.10)', borderRadius: 8 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {([
              { id: 'user' as Role, label: 'Customer', icon: '👤', desc: 'Submit & track your transactions', accent: '#38bdf8' },
              { id: 'admin' as Role, label: 'Analyst', icon: '🛡', desc: 'Manage fraud detection & alerts', accent: '#f59e0b' },
            ] as const).map(opt => (
              <button
                key={opt.id}
                type="button"
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                onBlur={e => (e.target.style.borderColor = 'rgba(148,163,184,0.12)')}
              />
              <button type="button" onClick={() => setShowPw(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: 0 }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 8s2.5-5 6-5 6 5 6 5-2.5 5-6 5-6-5-6-5Z" stroke="currentColor" strokeWidth={1.5}/><circle cx="8" cy="8" r="1.5" stroke="currentColor" strokeWidth={1.5}/>{!showPw && <path d="M2 2l12 12" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round"/>}</svg>
              </button>
            </div>
          </div>

          {error && (
            <div style={{ fontFamily: 'Instrument Sans', fontSize: 12, color: '#ef4444', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 4, padding: '8px 12px' }}>{error}</div>
          )}
          ...
          <button
            ...
            disabled={loading}
            style={{
              marginTop: 4, padding: '12px', border: 'none', borderRadius: 4,
              fontFamily: 'Instrument Sans', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer',
        ...
        <div style={{ marginTop: 24, paddingTop: 18, borderTop: '1px solid rgba(148,163,184,0.07)', display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
          {['JWT Auth', '256-bit AES', 'SOC 2 Type II'].map(t => (
            <span key={t} style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: '#4b5563', background: 'rgba(148,163,184,0.05)', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 3, padding: '3px 8px' }}>{t}</span>
          ))}
        </div>
        <p style={{ fontFamily: 'Instrument Sans', fontSize: 11, color: '#4b5563', textAlign: 'center', marginTop: 10 }}>Demo: any email and password</p>
      </div>
    </div>
  )
}
