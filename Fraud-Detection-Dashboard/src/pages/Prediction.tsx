...
const INIT: FormState = { amount: '', device: 'PC', location: 'United States', txType: 'Purchase', time: 'daytime', recurring: 'no', velocity: '1', country: 'US' }

const HIGH_RISK_COUNTRIES = ['NG', 'PA', 'RO', 'RU', 'VE']
const HIGH_RISK_DEVICES = ['Unknown']
const HIGH_RISK_TYPES = ['Crypto', 'Transfer', 'ATM']

function computeScore(f: FormState): number {
  let score = 10
  ...
  if (f.recurring === 'no' && amt > 500) score += 6
...
export default function Prediction() {
  ...
  const [result, setResult] = useState<null | { score: number; fraud: boolean }>(null)
  ...
  const color = result ? riskColor(result.score) : '#64748b'
  const level = result ? riskLevel(result.score) : 'safe'
  const circumference = 2 * Math.PI * 52
  const offset = result ? circumference - (result.score / 100) * circumference : circumference

  const selectStyle = {
    width: '100%', padding: '10px 12px', background: 'rgba(8,13,26,0.8)',
  ...
  const labelStyle = { fontFamily: 'Instrument Sans', fontSize: 12, fontWeight: 600 as const, color: '#94a3b8', letterSpacing: '0.04em', display: 'block' as const, marginBottom: 6 }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
          ...
          <form onSubmit={predict} style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <input type="number" min="0" placeholder="e.g. 5000" value={form.amount} onChange={set('amount')} style={inputStyle} required />
              </div>
              <div>
                <label style={labelStyle}>Velocity (tx/hr)</label>
                <input type="number" min="1" max="50" placeholder="1" value={form.velocity} onChange={set('velocity')} style={inputStyle} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <select value={form.country} onChange={set('country')} style={selectStyle}>
                  {[['US', 'United States'], ['GB', 'United Kingdom'], ['DE', 'Germany'], ['FR', 'France'], ['NG', 'Nigeria ⚠'], ['PA', 'Panama ⚠'], ['RO', 'Romania ⚠'], ['RU', 'Russia ⚠'], ['MX', 'Mexico'], ['CA', 'Canada'], ['AU', 'Australia']].map(([c, l]) => <option key={c} value={c}>{l}</option>)}
                </select>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{ flex: 1, padding: '12px', background: loading || !form.amount ? 'rgba(245,158,11,0.4)' : '#f59e0b', border: 'none', borderRadius: 4, fontFamily: 'Instrument Sans', fontSize: 14, fontWeight: 700, color: '#0a0f1e', cursor: loading || !form.amount ? 'not-allowed' : 'pointer', transition: 'background 0.15s' }}
              >
        ...
        {/* Result panel */}
        <div style={{ background: '#0d1528', border: `1px solid ${result ? (result.fraud ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.2)') : 'rgba(148,163,184,0.08)'}`, borderRadius: 4, padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, transition: 'border-color 0.3s' }}>

          {!result && !loading && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 40, color: '#1e2d4a', marginBottom: 12 }}>◎</div>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              ...
              <div style={{ fontFamily: 'Instrument Sans', fontSize: 12, color: '#64748b', marginTop: 8 }}>Running GBM model v4.2</div>
              <div style={{ marginTop: 20, display: 'flex', gap: 4, justifyContent: 'center' }}>
                {[0,1,2,3].map(i => (
                  <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', opacity: 0.3, animation: `bounce 1s ${i * 0.15}s infinite` }} />
                ))}
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div style={{ width: '100%' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    { label: 'Amount', v: Math.min(parseFloat(form.amount) / 200, 100) },
                    ...
                    { label: 'Tx Type Risk', v: HIGH_RISK_TYPES.includes(form.txType) ? 75 : 20 },
                    ...
                    return (
                      <div key={label}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                          ...
                          <span style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: riskColor(val) }}>{val}</span>
                        </div>
                        <div style={{ height: 5, background: 'rgba(148,163,184,0.08)', borderRadius: 2, overflow: 'hidden' }}>
                          <div style={{ width: `${val}%`, height: '100%', background: riskColor(val), borderRadius: 2, transition: 'width 0.6s ease' }} />
                        </div>
                      </div>
                    )
                  })}
