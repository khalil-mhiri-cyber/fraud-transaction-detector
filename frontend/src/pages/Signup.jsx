    ...
    <div style={{ minHeight: '100vh', background: '#080d1a', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', width: 460, padding: '48px 40px', background: '#0d1528', border: '1px solid rgba(148,163,184,0.10)', borderRadius: 8 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontFamily: 'Instrument Sans', fontSize: 12, fontWeight: 600, color: '#64748b', letterSpacing: '0.04em', marginBottom: 10 }}>CREATE ACCOUNT AS</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              { id: 'user', label: 'Customer', icon: '👤', desc: 'Submit & track transactions', accent: '#38bdf8' },
              { id: 'admin', label: 'Analyst', icon: '🛡', desc: 'Manage fraud detection', accent: '#f59e0b' },
            ].map(opt => (
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ fontFamily: 'Instrument Sans', fontSize: 12, fontWeight: 600, color: '#94a3b8', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>Full name</label>
          ...
          <div>
            ...
            <input type="email" placeholder={isAdmin ? 'analyst@sentinelai.io' : 'john.doe@email.com'}
          ...
          <div>
            <label style={{ fontFamily: 'Instrument Sans', fontSize: 12, fontWeight: 600, color: '#94a3b8', letterSpacing: '0.04em', display: 'block', marginBottom: 6 }}>Confirm password</label>
            <input type={showPw ? 'text' : 'password'} placeholder="••••••••••••"
          ...
          <button type="submit" disabled={loading} style={{
            ...
            color: '#0a0f1e', transition: 'background 0.2s',
          }}>
            {loading ? 'Creating account...' : `Create ${isAdmin ? 'Analyst' : 'Customer'} account`}
          </button>
        </form>

        <div style={{ marginTop: 20, textAlign: 'center' }}>
          <span style={{ fontFamily: 'Instrument Sans', fontSize: 13, color: '#64748b' }}>Already have an account? </span>
          <Link to="/login" style={{ fontFamily: 'Instrument Sans', fontSize: 13, color: isAdmin ? '#f59e0b' : '#38bdf8', textDecoration: 'none', fontWeight: 600 }}>
            Sign in
          </Link>
        </div>
        ...
        <div style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid rgba(148,163,184,0.07)', display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
          {['Encrypted', 'Secure', 'Protected'].map(t => (
