...
export default function Settings() {
  ...
  const inputStyle = { width: '100%', padding: '10px 14px', background: 'rgba(8,13,26,0.8)', border: '1px solid rgba(148,163,184,0.10)', borderRadius: 4, fontFamily: 'Instrument Sans', fontSize: 13, color: '#e2e8f0', outline: 'none', boxSizing: 'border-box' };
  const labelStyle = { fontFamily: 'Instrument Sans', fontSize: 12, fontWeight: 600, color: '#94a3b8', display: 'block', marginBottom: 6 };
  ...
  function ToggleRow({ label, desc, on, setOn }) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderTop: '1px solid rgba(148,163,184,0.06)' }}>
        ...
        <button onClick={() => setOn(!on)} style={{ width: 44, height: 24, borderRadius: 12, background: on ? '#f59e0b' : 'rgba(148,163,184,0.15)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
  ...
  const tabs = [{ id: 'profile', label: 'Profile' }, { id: 'rules', label: 'Detection Rules' }, { id: 'notifications', label: 'Notifications' }, { id: 'api', label: 'API & Access' }];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 760 }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid rgba(148,163,184,0.08)' }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ fontFamily: 'Instrument Sans', fontSize: 13, fontWeight: tab === t.id ? 600 : 400, color: tab === t.id ? '#e2e8f0' : '#64748b', background: 'none', border: 'none', cursor: 'pointer', padding: '10px 16px', borderBottom: tab === t.id ? '2px solid #f59e0b' : '2px solid transparent', marginBottom: -1, transition: 'all 0.15s' }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === 'rules' && (
        <div style={{ background: '#0d1528', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 4, padding: 28 }}>
          ...
          <ToggleRow label="Auto-Block High Risk" desc="Automatically block transactions above the threshold" on={autoBlock} setOn={setAutoBlock} />
          <ToggleRow label="Velocity Check" desc="Flag accounts with 5+ transactions per hour from same IP" on={true} setOn={() => {}} />
          <ToggleRow label="Geo Anomaly Detection" desc="Alert on impossible travel patterns" on={true} setOn={() => {}} />
      ...
      {tab === 'notifications' && (
        <div style={{ background: '#0d1528', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 4, padding: 28 }}>
          ...
          <ToggleRow label="Email Alerts" desc="Send critical and high severity alerts to your email" on={alertEmail} setOn={setAlertEmail} />
          <ToggleRow label="Slack Integration" desc="Post alerts to your team's Slack channel" on={alertSlack} setOn={setAlertSlack} />
          <ToggleRow label="Daily Digest" desc="Receive a daily summary of fraud activity each morning" on={true} setOn={() => {}} />
          <ToggleRow label="Weekly Report" desc="Automated report every Monday with trends and KPIs" on={false} setOn={() => {}} />
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: '#0d1528', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 4, padding: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <input value="sk-sentinel-••••••••••••••••••F2A1" readOnly style={{ ...inputStyle, fontFamily: 'JetBrains Mono', fontSize: 12, flex: 1 }} />
          ...
          <div style={{ background: '#0d1528', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 4, padding: 28 }}>
            ...
            <div style={{ fontFamily: 'JetBrains Mono', fontSize: 12, color: '#94a3b8', background: 'rgba(8,13,26,0.6)', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 4, padding: '12px 16px', lineHeight: 1.8 }}>
              <span style={{ color: '#10b981' }}>POST</span> <span style={{ color: '#38bdf8' }}>http://localhost:8000/predict</span><br />
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <button onClick={save} style={{ padding: '11px 24px', background: '#f59e0b', border: 'none', borderRadius: 4, fontFamily: 'Instrument Sans', fontSize: 14, fontWeight: 700, color: '#0a0f1e', cursor: 'pointer' }}>Save Changes</button>
