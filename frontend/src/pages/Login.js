...
function Login() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const userData = { email, name: email.split('@')[0], role: role.toUpperCase() };
      // Store JWT token
    ...
    <div style={{ minHeight: '100vh', background: '#080d1a', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', width: 460, padding: '48px 40px', background: '#0d1528', border: '1px solid rgba(148,163,184,0.10)', borderRadius: 8 }}>
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {error && <div style={{ fontFamily: 'Instrument Sans', fontSize: 12, color: '#ef4444', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 4, padding: '8px 12px' }}>{error}</div>}
        ...
        <div style={{ marginTop: 20, textAlign: 'center' }}>
          ...
          <Link to="/signup" style={{ fontFamily: 'Instrument Sans', fontSize: 13, color: isAdmin ? '#f59e0b' : '#38bdf8', textDecoration: 'none', fontWeight: 600 }}>Create account</Link>
        </div>
